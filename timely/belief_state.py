...
class BeliefState(object):
    ...
    def __init__(self, dataset, actions, mode='fixed_order', bounds=None, model=None, fastinf_model_name='perfect'):
        ...
        if self.gist_mode:
            ...
            self.num_obs_vars = len(
                self.actions) - 1 + len(self.dataset.classes)
        ...
        if mode == 'random':
            ...
            if model:
                ...
                self.model = FastinfModel(
                    dataset, self.fastinf_model_name, self.num_obs_vars)
        else:
    ...
    def update_with_score(self, action_ind, score):
        ...
        if self.mode in ['random', 'fixed_order']:
            ...
            self.model.update_with_observations(
                self.observed, self.observations)
    ...
    def compute_full_feature(self):
        ...
        h_c[h_c == -0] = 0
        time_to_start_ratio = 0 if self.t >= self.bounds[
            0] else self.t / (self.bounds[0])
